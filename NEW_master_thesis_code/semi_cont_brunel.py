"""
This file contains the simulation of the Brunel model when the neurons and everything else is continuous, however
the delays are discrete.
"""
...
def build_network():
    ...
    recorder_label = os.path.join(
        params['path_name'],
        'brunel_semi_continuous' + '_seed_' + str(params['rng_seed']) + '_spikes_exc')
    ...
    recorder_label_i = os.path.join(
        params['path_name'],
        'brunel_semi_continuous_' + '_seed_' + str(params['rng_seed']) + '_spikes_inh')
    ...
    # we want everything else to be continuous but the delays, so we can se the delays directly in the connections
    split = int(1/nest.resolution)
    ...
    nest.Connect(nodes_ex, nodes_ex + nodes_in, conn_params_ex,
                 syn_spec={'weight': J_ex, 'delay': 1.0+nest.resolution*nest.random.uniform_int(split+1)})
    ...
    nest.Connect(nodes_in, nodes_ex + nodes_in, conn_params_in,
                 syn_spec={'weight': J_in, 'delay': 1.0+nest.resolution*nest.random.uniform_int(split+1)})

    conn_ex = nest.GetConnections(source=nodes_ex)
    conn_in = nest.GetConnections(source=nodes_in)
