...
"""
RUn wasserstein distance on continuous model seeds 2-10 vs cont seed 1.
"""
...
rest_cont = [f'/p/home/jusers/groenbekk1/jusuf/project/wasserstein_calculations/organized_spike_data/resolution_1_8/brunel_continuous/brunel_continuous_delay_1.0_2.0_seed_{i}_spikes_exc-12502-0.dat'.format(i) for i in range(2,11)]

# Create classes for each type of model

rest_cont_classes = get_classes(rest_cont)

# Create instances of each class

rest_cont_instances = create_instances(rest_cont_classes)

# Get the Wasserstein distances
ws_rest_cont = get_ws_distances(rest_cont_instances)


print('res cont')
print(ws_rest_cont)
t = pd.DataFrame(ws_rest_cont, index = [2,3,4,5,6,7,8,9,10])
t.to_csv('ws_cc_rest_cont.csv')
