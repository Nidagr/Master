            ...
            if self.nest_version == '3':
                sd_dict = {
                    ...
                    'label': os.path.join(
                        self.data_path,
                        'spike_recorder'),
                    'precision': 12  # added this precision to spike recorders
                }
            ...
            for j, source_pop in enumerate(self.pops):
                if self.num_synapses[i][j] >= 0.:
                    ...
                    if self.nest_version == '3':
                        ...
                        # define the min and max values of distribution from which delays are drawn
                        delay_min = self.net_dict['delay_matrix_mean'][i][j] - self.net_dict['delay_matrix_mean'][i][j] \
                                    * self.net_dict['delay_rel_std']
                        delay_max = self.net_dict['delay_matrix_mean'][i][j] + self.net_dict['delay_matrix_mean'][i][j] \
                                    * self.net_dict['delay_rel_std']
                        # the min and max value for the uniform distribution changes whether we want the droop or equal
                        # model, need to use an if-test and maybe take in a parameter from e.g. network_params about
                        # if the model is equal or droop, if droop use the below, if equal change min and max according
                        # to resolution
                        if self.net_dict['network_type'] == 'droop':
                            syn_dict = {
                                # we let weight stay as it was
                                ...
                                'weight': nest.math.redraw(
                                    ...
                                    min=w_min,
                                    max=w_max),
                                # we want the delays to be drawn from uniform distribution, change code here
                                'delay': nest.random.uniform(delay_min, delay_max)}
                        elif self.net_dict['network_type'] == 'equal':
                            if self.sim_dict['sim_resolution'] == 1 / 4:
                                # if the resolution is 1/4, to get equal probabilities we need to use [min,max] for
                                # inhibitory neurons, and use [min-h/2,max+h/2] for excitatory neurons.
                                if delay_min == (self.net_dict['delay_inh_mean'] - self.net_dict['delay_inh_mean']
                                                 * self.net_dict['delay_rel_std']):
                                    # we are dealing with inhibitory delay, use the standard [min,max] interval
                                    syn_dict = {
                                        # we let weight stay as it was
                                        'synapse_model': self.net_dict['synapse_type'],
                                        'weight': nest.math.redraw(
                                        ...
                                        # we want the delays to be drawn from uniform distribution, change code here
                                        'delay': nest.random.uniform(delay_min, delay_max)}
                                elif delay_min == (self.net_dict['delay_exc_mean'] - self.net_dict['delay_exc_mean']
                                                   * self.net_dict['delay_rel_std']):
                                    # we are dealing with excitatory delay, use [min-h/,max+h/2] interval
                                    syn_dict = {
                                        # we let weight stay as it was
                                        'synapse_model': self.net_dict['synapse_type'],
                                        'weight': nest.math.redraw(
                                            ...
                                            min=w_min,
                                            max=w_max),
                                        # we want the delays to be drawn from uniform distribution, equal probability of
                                        # each value being drawn
                                        'delay': nest.random.uniform(delay_min - self.sim_dict['sim_resolution'] / 2,
                                                                     delay_max + self.sim_dict['sim_resolution'] / 2)}
                            else:
                                # not resolution 1/4
                                syn_dict = {
                                    # we let weight stay as it was
                        ...
                        # do not care about changing this as we use nest version 3
                        syn_dict = {
